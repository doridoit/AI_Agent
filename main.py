...
from modules.components.llm_selector import LLMManager
from modules.chatbot import ask_with_llm, create_memory_conversational_chain, extract_analysis_intent_and_column
from modules.processing import eda, anomaly_model

st.set_page_config(page_title="Data Analysis AI Agent", layout="wide")
st.title("🖥️ Data Analysis AI Agent")

# 앱이 처음 실행될 때만 LLMManager 인스턴스를 세션 상태에 저장합니다.
...
def show_chatbot_section(df):
    ...
    # LLM 초기화 로직
    # 세션에 LLM 인스턴스가 없거나, 선택된 모델이 변경되었을 경우 초기화
    if "llm" not in st.session_state or st.session_state["last_loaded_model"] != selected_provider:
        result = st.session_state.llm_manager.init(selected_provider)
        if result == "ok":
            st.session_state["llm"] = st.session_state.llm_manager.get_llm()
            st.session_state["last_loaded_model"] = selected_provider
            st.success(f"✅ LLM 초기화 완료 ({selected_provider})")
        else:
            st.error(f"❌ LLM 초기화 실패: {st.session_state.llm_manager.get_error()}")
...
if uploaded_file:
    ...
    if file_ext == ".csv":
        ...
        try:
            ...
            selected_sheet = st.selectbox("📑 시트를 선택하세요", sheet_names, key="excel_sheet_selectbox")
            df_preview = pd.read_excel(xls, sheet_name=selected_sheet, engine="openpyxl", nrows=10)
            st.write("🔎 시트 미리보기 (상위 10행)")
            st.dataframe(df_preview)

            start_row = st.number_input("데이터가 시작되는 행 번호를 입력하세요 (0부터 시작)", min_value=0, value=0, step=1, key="start_row_input")
    ...
    if df is not None:
        # 2. EDA 분석
        eda_type = st.radio("📌 EDA 유형을 선택하세요", ("일반형 EDA", "다변형 EDA"), key="eda_radio")
        ...
        selected_model = st.selectbox("모델을 선택하세요", ["Logistic Regression", "XGBoost", "AutoEncoder"], key="anomaly_model_selectbox")
